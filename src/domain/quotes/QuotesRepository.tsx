...
class QuotesRepository {
  async findAll() {
    ...
    return rows.map((row: any) => new Quote(row.id, row.quote, row.author, row.authorId, row.avatar, row.channel));
  }

  async random(authorId: string) {
    const connection = await Mysql.getConnection();
    let results;
    ...
    if (authorId) {
      [
        ...
        'SELECT quotes.id, quotes.quote, COALESCE(authors.name, quotes.author) as author, quotes.authorId, COALESCE(authors.avatar, quotes.avatar) as avatar, quotes.channel ' +
        'FROM quotes ' +
        'LEFT JOIN authors ON authors.id = quotes.author_id ' +
    ...
    return new Quote(row.id, row.quote, row.author, row.authorId, row.avatar, row.channel);
  }
